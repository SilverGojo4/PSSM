...
You can run each stage manually via the CLI with detailed control over input/output paths and settings.
...
### 🔹 PSI-BLAST Stage (PSSM Generation)
...
After running the pipeline, the outputs are organized as follows:
...
Each `.pssm` file represents the position-specific scoring matrix generated by PSI-BLAST for a given sequence, while the corresponding `_hits.tsv` file contains its matched domains from the CDD.
